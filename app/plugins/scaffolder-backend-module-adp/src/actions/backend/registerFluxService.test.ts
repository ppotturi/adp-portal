...
import { registerFluxServiceAction } from './registerFluxService';
...
type ActionIO =
  Parameters<
    ReturnType<typeof registerFluxServiceAction>['handler']
  >[0] extends ActionContext<infer Input, infer Output>
    ? { input: Input; output: Output }
    : never;

describe('registerFluxServiceAction', () => {
  function setup() {
    ...
    return {
      sut: registerFluxServiceAction({ config, fetchApi, tokens }),
      config,
      fetchApi,
      tokens,
    };
  ...
  it('Should call the correct endpoint when invoked', async () => {
    ...
    expect(fetchApi.fetch).toHaveBeenCalledWith(
      'https://localhost:1234/adp-csharp-api/FluxTeamConfig/my-team/services',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
